    ...
    self.fs.stat(fspath, function(err, stats) {
        if (err) {
            if (err.errno != constants.ENOENT)
                self._logIf(0, "Error other than ENOENT in call to 'stat'" + err, self);
...
FtpConnection.prototype._PASV = function (commandArg, command) {
    ...
    if (self.dataSocket) {
        self.dataSocket.end(function () { // close any existing connections
            self.dataSocket = null;
            onceClosed();
        });
    }
...
FtpConnection.prototype._setupNewPASV = function (commandArg, command) {
    ...
    if (self.server.options.pasvPortRangeStart && self.server.options.pasvPortRangeEnd) {
        ...
        portRangeErrorHandler = function (e) {
            if (e.errno == constants.EADDRINUSE && i < self.server.options.pasvPortRangeEnd)
                self.pasv.listen(++i);
        };
    ...
    self.pasv.on("error", function (err) {
        self._logIf(0, "Error attempting to listen for passive connection: " + err, self);
    ...
    if (self.server.options.slurpFiles) {
        ...
        self.fs.open(filename, "r", function (err, fd) {
            if(err) {
                ...
                self._logIf(0, "DATA file " + filename + " opened", self);
                afterOk(function () {
                    readChunk();
                });
            }
                ...
                self._whenDataReady(function (pasvconn) {
                    ...
                    self.fs.read(fd, self.buffer, 0, 4096, null/*pos*/, function(err, bytesRead, buffer) {
                        ...
                        if (bytesRead > 0) {
                            ...
                            self._logIf(0, "DATA file " + filename + " closed", self);
