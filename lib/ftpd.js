...
FtpConnection.prototype._createPassiveServer = function () {
    ...
    return net.createServer(function (psocket) {
        ...
        function setupPassiveListener() {
            ...
            self.dataSocket.on("end", function (had_error) {
                self._logIf(
                    (had_error ? 0 : 3),
                    "Passive data event: end " + (had_error ? " due to error" : "")
                );
...
FtpConnection.prototype._onError = function (err) {
    this._logIf(0, "Client connection error: " + err);
    this.socket.destroy();
};

FtpConnection.prototype._onEnd = function () {
    this._logIf(1, "Client connection ended");
};
		    ...
		    return;
                ...
                for (var i = 0; i < files.length; ++i) {
                    ...
                    self.server.getGroupFromGid(file.stats.gid, function (e2, gname) {
                        if (e1) self._logIf(0, "While attempting to get username: " + e1);
                        if (e2) self._logIf(0, "While attempting to get group:" + e2);
                        if (e1 || e2) return;
                        ...
                        wwenc(pasvconn, line, (i == files.length - 1 ? success : undefined));
                    });});
		}
	    });
        });
    ...
    wwenc(self.socket, "150 Here comes the directory listing\r\n", function () {
	    ...
	    glob.setMaxStatsAtOnce(self.server.options.maxStatsAtOnce);
            glob.glob(PathModule.join(self.root, temp), self.fs, function(err, files) {
                if (err) {
                    self._logIf(0, "During NLST, error globbing files: " + err);
                ...
                self._logIf(3, "Directory has " + files.length + " files");
                if (files.length > 0) {
                    wwenc(pasvconn, files.map(function (f) { return f.name })
                                         .join("\r\n") + "\r\n", success);
                }
                else {
                    success();
                }
            }, self.server.options.maxStatsAtOnce);
        });
    });
};
...
FtpConnection.prototype._writePASVReady = function (command) {
    ...
    var host = self.dataHost;
    var port = self.dataPort;
    if (command == "PASV") {
...
FtpConnection.prototype._setupNewPASV = function (commandArg, command) {
    ...
    self.pasv = self._createPassiveServer();
                ...
                function writeBuf(wbuf, upto) {
                    ...
                    self.fs.write(fd, buf, 0, upto, null, function (err) {
                        if (err) {
                            ...
                            self.fs.close(fd, function (err) {
                                self._logIf(0, "Error closing file following write error");
                            });
        ...
        self.dataSocket.once('error', function (buf) {
            ...
            self.fs.close(fd, function (err) {
                if (err) {
                    if (self.dataSocket) // dataSocket will have been set to null if 'end' event was raised.
                        self.dataSocket.removeListener('data', dataHandler);
                    self._logIf(0, "Error closing file following error on dataSocket");
                    wwenc(self.socket, "426 Connection closed; transfer aborted\r\n");
                }
            });
        });
        self.dataSocket.once('end', function () {
            if (erroredOut)
                return;
            ...
            if (slurpBuf) {
                ...
                self.fs.write(fd, slurpBuf, 0, totalBytes, null, function (err) {
                    if (err) {
                        self._logIf(0, "Error writing slurp buffer to file following 'end' message");
                        wwenc(self.socket, "426 Connection closed; transfer aborted\r\n");
                        return;
                    }
            ...
            function onceOnDisk() {
                self.fs.close(fd, function (err) {
                    if (err) {
                        self._logIf(0, "Error closing file following 'end' message");
                        wwenc(self.socket, "426 Connection closed; transfer aborted\r\n");
                        return;
                    }
