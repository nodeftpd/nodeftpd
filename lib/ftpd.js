...
FtpConnection.prototype.closeDataConnections = function () {
...
FtpConnection._onError = function () {
    lofIf(0, "Client connection error: " + err, this.socket);
    this.socket.destroy();
};

FtpConnection._onEnd = function () {
    logIf(1, "Client connection ended", this.socket);
};

FtpConnection._onConnect = function () {
...
FtpConnection._onData = function (data) {
    var self = this;
...
FtpConnection.prototype.onPassiveConnection = function (socket) {
...
FtpConnection._command_NOOP = function () {
    // No operation (dummy packet; used mostly on keepalives).
    wwenc(this.socket, "200 OK\r\n");
};

FtpConnection._command_PASS = function () {
    var self = this;
    ...
    self.server.emit(
        ...
        function(username, userFsModule) { // implementor should call this on successful password check
            ...
            if (userFsModule)
                ...
                self.fs = FsModule;
            self.cwd = self.getInitialCwd(username);
            self.root = self.getRoot(username);
        },
...
FtpConnection._command_PASV = function (x, y) { this._PASV(x, y); };
FtpConnection._command_EPASV = function (x, y) { this._PASV(x, y); };
FtpConnection._PASV = function (commandArg, command) {
    var self = this;
    ...
    if (self.options.pasvPortRangeStart && self.options.pasvPortRangeEnd) {
        ...
        var i = self.options.pasvPortRangeStart;
        self.pasv.listen(i);
...
FtpConnection.protoype._command_RETR = function (commandArg) {
        ...
        self._whenDataReady( function(pasvconn) {
            ...
            if (self.options.slurpFiles) {
                self.fs.readFile(self.filename, function (err, contents) {
                    if (err) {
                        if (err.code == 'ENOENT') {
                            ...
                            traceIf(0, "Error at read other than ENOENT " + err, self);
                        }
                    }
                    else {
                ...
                self.fs.open(self.filename, "r", function (err, fd) {
                    logIf(0, "DATA file " + self.filename + " opened", self);
                    function readChunk() {
...
FtpConnection.prototype._command_RMD = function (commandArg) {
    ...
    self.fs.rmdir( filename, function(err){
        if(err) {
            logIf(0, "Error removing directory " + filename, self.socket);
...
FtpConnection.prototype._command_RNFR = function (commandArg) {
    ...
    logIf(3, "Rename from " + self.filefrom, self.socket);
...
FtpConnection.prototype._command_SIZE = function (commandArg) {
    ...
    if (!self._authenticated())
        break;
    var filename = PathModule.join(self.root, withCwd(self.cwd, commandArg));
    self.fs.stat( filename, function (err, s) {
        if(err) { 
...
FtpConnection.prototype._command_STOR = function (commandArg) {
    ...
    var filename = PathModule.join(self.root, withCwd(self.cwd, commandArg));
    var fd;
    self.fs.open( filename, 'w', 0644, function(err, fd_) {
        fd = fd_;
        if(err) {
