...
function statList(fsm, list, callback) {
  ...
  var stats = [];
  var i = 0;
  var count = Math.min(list.length, CONC);
  ...
  function handleFile(index) {
    ...
    if (i >= list.length) {
      return stats.length === list.length ? finished() : null;
    }

    i++;
    fsm.stat(list[index], function(err, st) {
      if (err) {
        erroredOut = true;
        callback(err);
      } else {
        stats.push({
          name: PathModule.basename(list[index]),
          stats: st
        });
        handleFile(i);
      }
